...
fun main() {
    printDivisionProcess(240842, 1625, "temp.txt")
}
 ...
 * Пустые строки во входном файле обозачают конец абзаца,
...
fun printDivisionProcess(lhv: Int, rhv: Int, outputName: String) {
    ...
    for (i in result.indices) {
        if (i == 0) {
            ...
            index++
            numOfSpaces += if (listOfDifferences[i].toString().length == 4) numDown.length - listOfDifferences[i].toString().length - 1
